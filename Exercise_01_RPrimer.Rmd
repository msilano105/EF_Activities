...
```
If the output of the equation is not a number than it will appear as NaNs
```

2.	Why are we multiplying the results of the atan and atan2 functions by 180/pi? 
```
so then the results will be in detgrees instead of radians.
```

3.	How is the atan2 function different from the atan function? 
```
atan2 does two arguments 
```

4.	What is the difference between log and log10?
```
log is the natural log and log10 is log bas 10
```

5.	Given a right triangle with sides x = 5 and y = 13, calculate the length of the hypotenuse (show code)
```
We know that the longest side of a triangle is a^2 + b^2 = c^2
therefore the length of the hpyptenuse will be 5^2 + 13^2
or 13.928
```
...
```
the difference in value is that the -6 to 6 is simply even numbers but when doing the second sequence we can see that R picks 8 numbers from the two specefied ends by being evenly distributed.
```
b.	What is the sum of the exponential of this sequence?
```{r}
sum(seq(-4.8, -3.43, length=8))
```
...
21.  Make a barplot of the mean number of frogs
```{r}
barplot(mean(dat$frogs))
```

22.	Make a barplot of the median number of frogs grouped by whether they have spots or not.
```{r}
barplot(tapply(dat$frogs, dat$spots, median))
```
 
23..	Plot a histogram of blue frogs
```{r}
hist(datblue$frogs, xlab = "Amount of Blue Frogs", main = "Histogram of Blue Frogs")
```


24.	Use apply to calculate the across-population standard deviations in the numbers of frogs and tadpoles
```{r}
apply(dat[,1:2],2,sd)
```
...
```
```{r}
if(all(x>=0)){plot(dat$frogs)}else{hist(dat$frogs)}
```
...
```
```{r}
myplot <- function(dat){plot(dat$frogs,dat$tadpoles,
     cex=1.5,    		# increase the symbol size
     col=as.character(dat$color),	# change the symbol color by name
     pch=dat$spots+1,			# change the symbol (by number)
     cex.axis=1.3,			# increase the font size on the axis 
