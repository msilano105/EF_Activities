...
abline(v=1)					## add a line to indicate the mean (“v” is for “vertical”)
...
lines(x,dexp(x,.5),col=3)	

plot(x, pexp(x), type = "l")
lines(x,pexp(x,2),col=2)			
lines(x,pexp(x,.5),col=3)	

p = seq(0,1,by=0.01)
plot(p,qexp(p,1,1),type='l',ylim=range(x))    
lines(p,qexp(p,2,1),col=2)
lines(p,qexp(p,3,.5),col=3)

qexp(c(0.025,0.975),1)		
plot(p,qexp(p,0,1),type='l',ylim=range(x))
abline(v=c(0.025,0.975),lty=2)	
abline(h=qexp(c(0.025,0.975)),lty=2)	

plot(x,dexp(x,1),type='l')	
abline(v=qexp(c(0.025,0.975),1),lty=2)

n = c(10,100,1000,10000)	# sequence of sample sizes
for(i in 1:4){			# loop over these sample sizes
  hist(rexp(n[i]),main=n[i],probability=TRUE,breaks=40)  
				#here breaks defines number of bins in the histogram
...
```
I am not sure why lnorm is not a function even though that is what you said to call it as in the chart above.  
```
