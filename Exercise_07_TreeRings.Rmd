...
model{
  ...
  ## compile JAGS model
  j.model.1   <- jags.model (file = textConnection(TreeDataFusionMV.1),
                           data = data,
                           inits = init,
                           n.chains = 3)
  ## burn-in
  jags.out.1   <- coda.samples (model = j.model.1,
                              variable.names = c("tau_add","tau_dbh","tau_inc","mu","tau_ind","tau_yr"),
                              n.iter = min(n.iter.1,2000))
  plot(jags.out.1)
  
  ## run MCMC
  jags.out.1   <- coda.samples (model = j.model.1,
                              variable.names = c("x","tau_add","tau_dbh","tau_inc","mu",
                                                 "tau_ind","tau_yr","ind","year"), n.iter = n.iter.1)
```
...
```{r, fig.height=8}
  ...
  ### DBH
  layout(matrix(1:8,4,2))
  out.1 <- as.matrix(jags.out.1)
  x.cols.1 = which(substr(colnames(out.1),1,1)=="x")   
  ci.1 <- apply(out.1[,x.cols.1],2,quantile,c(0.025,0.5,0.975))
  ci.names.1 = parse.MatrixNames(colnames(ci.1),numeric=TRUE)
  
  smp = c(sample.int(data$ni,3),49)
  for(i in smp){
    sel.1 = which(ci.names.1$row == i)
    plot(data$time,ci.1[2,sel.1],type='n',ylim=range(ci.1[,sel.1],na.rm=TRUE),ylab="DBH (cm)",main=i)
    ciEnvelope(data$time,ci.1[1,sel.1],ci.1[3,sel.1],col="green")
    points(data$time,data$z[i,],pch="+",cex=1.5)
  }
  ...
  for(i in smp){
    sel.1 = which(ci.names.1$row == i)
    inc.mcmc.1 = apply(out.1[,x.cols.1[sel.1]],1,diff)
    inc.ci.1 = apply(inc.mcmc.1,1,quantile,c(0.025,0.5,0.975))*5
    
    plot(data$time[-1],inc.ci.1[2,],type='n',ylim=range(inc.ci.1,na.rm=TRUE),ylab="Ring Increment (mm)")
    ciEnvelope(data$time[-1],inc.ci.1[1,],inc.ci.1[3,],col="green")
    points(data$time,data$y[i,]*5,pch="+",cex=1.5,type='b',lty=2)
  }
```  
...
```{r}
  ## process model
  vars.1 = (1:ncol(out.1))[-c(which(substr(colnames(out.1),1,1)=="x"),grep("tau",colnames(out.1)),
                          grep("year",colnames(out.1)),grep("ind",colnames(out.1)))]
  par(mfrow=c(1,1))
  for(i in vars.1){
    hist(out.1[,i],main=colnames(out.1)[i])
  }
  if(length(vars.1)>1) pairs(out.1[,vars.1])

  ## Standard Deviations
  #layout(matrix(c(1,2,3,3),2,2,byrow=TRUE))
  par(mfrow=c(2,3))
  prec.1 = out.1[,grep("tau",colnames(out.1))]
  for(i in 1:ncol(prec.1)){
    hist(1/sqrt(prec.1[,i]),main=colnames(prec.1)[i])
  }
  cor(prec.1)
  pairs(prec.1)
```
...
TreeDataFusionMV.Random = "model{
  ...
  ## compile JAGS model
  j.model.Random   <- jags.model (file = textConnection(TreeDataFusionMV.Random),
                           data = data,
                           inits = init,
                           n.chains = 3)
  ## burn-in
  jags.out.Random   <- coda.samples (model = j.model.Random,
                              variable.names = c("tau_add","tau_dbh","tau_inc","mu","tau_ind","tau_yr"),
                              n.iter = min(n.iter.1,2000))
...
>>>>>>> c65e4d8af09ddc376dab5ff669557106d59feec0
...
```{r, fig.height=8}
  ...
  x.cols.Random = which(substr(colnames(out.Random),1,1)=="x")   
  ci.Random <- apply(out.Random[,x.cols.Random],2,quantile,c(0.025,0.5,0.975))
  ci.names.Random = parse.MatrixNames(colnames(ci.Random),numeric=TRUE)
  
  smp = c(sample.int(data$ni,3),49)
  for(i in smp){
    sel.Random = which(ci.names.Random$row == i)
    plot(data$time,ci.Random[2,sel.Random],type='n',ylim=range(ci.Random[,sel.Random],na.rm=TRUE),ylab="DBH (cm)",main=i)
    ciEnvelope(data$time,ci.Random[1,sel.Random],ci.Random[3,sel.Random],col="red")
    points(data$time,data$z[i,],pch="+",cex=1.5)
  }
  ...
  for(i in smp){
    sel.Random = which(ci.names.Random$row == i)
    inc.mcmc.Random = apply(out.Random[,x.cols.Random[sel.Random]],1,diff)
    inc.ci.Random = apply(inc.mcmc.Random,1,quantile,c(0.025,0.5,0.975))*5
    
    plot(data$time[-1],inc.ci.Random[2,],type='n',ylim=range(inc.ci.Random,na.rm=TRUE),ylab="Ring Increment (mm)")
    ciEnvelope(data$time[-1],inc.ci.Random[1,],inc.ci.Random[3,],col="red")
    points(data$time,data$y[i,]*5,pch="+",cex=1.5,type='b',lty=2)
  }
```  
...
```{r}
  ## process model
  vars.Random = (1:ncol(out.Random))[-c(which(substr(colnames(out.Random),1,1)=="x"),grep("tau",colnames(out.Random)),
                          grep("year",colnames(out.Random)),grep("ind",colnames(out.Random)))]
  par(mfrow=c(1,1))
  for(i in vars.Random){
    hist(out.Random[,i],main=colnames(out.Random)[i])
  }
  if(length(vars.Random)>1) pairs(out.Random[,vars.Random])

  ## Standard Deviations
  #layout(matrix(c(1,2,3,3),2,2,byrow=TRUE))
  par(mfrow=c(2,3))
  prec.Random = out.Random[,grep("tau",colnames(out.Random))]
  for(i in 1:ncol(prec.Random)){
    hist(1/sqrt(prec.Random[,i]),main=colnames(prec.Random)[i])
  }
  cor(prec.Random)
  pairs(prec.Random)
```
...
```{r}
  par(mfrow=c(1,1))
  ### YEAR
  year.cols.Random = grep("year",colnames(out.Random))
  if(length(year.cols.Random>0)){
    ci.yr.Random <- apply(out.Random[,year.cols.Random],2,quantile,c(0.025,0.5,0.975))
    plot(data$time,ci.yr.Random[2,],type='n',ylim=range(ci.yr.Random,na.rm=TRUE),main="Year Effect",ylab="cm")
    ciEnvelope(data$time,ci.yr.Random[1,],ci.yr.Random[3,],col="red")
    lines(data$time,ci.yr.Random[2,],lty=1,lwd=2)
    abline(h=0,lty=2)
  }
  
  ### INDIV
  ind.cols.Random= which(substr(colnames(out.Random),1,3)=="ind")
  if(length(ind.cols.Random)>0){
    boxplot(out.Random[,ind.cols.Random],horizontal=TRUE,outline=FALSE,col=combined$PLOT,main="Individual Effects By Plot",xlab="cm")
    abline(v=0,lty=2)
    ## calculate plot-level means for random effects
    tapply(apply(out.Random[,ind.cols.Random],2,mean),combined$PLOT,mean)
    table(combined$PLOT)
    
    spp = combined$SPP
    boxplot(out.Random[order(spp),ind.cols.Random],horizontal=TRUE,outline=FALSE,col=spp[order(spp)],main="Individual Effects By Species",xlab="cm")
    abline(v=0,lty=2)
    spp.code = levels(spp)[table(spp)>0]
    legend("bottomright",legend=rev(spp.code),col=rev(which(table(spp)>0)),lwd=4)
    ## calculate species-level means for random effects
    tapply(apply(out.Random[,ind.cols.Random],2,mean),combined$SPP,mean)
  }
```
...
```
When adding the random effect, the model seemed to preform better and the CI's were statistically significatly smaller.  Based on the diagnostics from the models i would add a random effect to DBH because we are attempting to draw indiviudals at random from a population so that they would be etimating the variability.    
```
...
```
I would use a violin plot to see the differences in variability from variable to variable.  Finding which data points add the most uncertaintity and add the most understaing could be visually understood beter with a violin plot.
...
Random.Code = "model{
  ...
  #### Data Model: DBH
  for(t in 1:nt){
  DBHnew[i,t] = x[i,t-1] + mu + ind[i] + year[t]
    z[i,t] ~ dnorm(DBHnew[i,t],tau_dbh)
  }
