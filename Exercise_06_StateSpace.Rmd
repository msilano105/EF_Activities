...
* Generate a time-series plot for the CI of x that includes the observations (as above). 
  Use a different color and symbol to differentiate observations that were included in the model versus       those that were converted to NA's.
```{r}
w = seq(1, 620, by=2)
x = seq(2, 620, by=4)
...
"
data1 <- list(y1=log(y1),n=length(y1),x_ic=log(1000),tau_ic=100,a_obs=1,r_obs=1,a_add=1,r_add=1)

j.model.1   <- jags.model (file = textConnection(RandomWalk1),
                             data = data1,
                             inits = init,
                             n.chains = 3)
...
ci.1 <- apply(exp(out.1[,3:ncol(out.1)]),2,quantile,c(0.025,0.5,0.975))
```

```{r}
plot(monthly.time,ci[2,],type='n',ylim=range(y1,na.rm=TRUE),ylab="Flu Index",log='y',xlim=range(monthly.time, na.rm=TRUE), xlab="Time") 
if(diff(time.rng) < 100){ 
  axis.Date(1, at=seq(monthly.time[time.rng[1]],monthly.time[time.rng[2]],by='month'), format = "%Y-%m")
}
ciEnvelope(monthly.time,ci[1,],ci[3,],col="green")
points(monthly.time,y1,pch="*",cex=0.5)
```

* Compare the CI between the two runs.
```
Because of the lack of data, that we created, the CI is much larger in comparison to the example plot. It should be obvious that this would happen.  Since the data still goes month to month though, the CI is not horrendously large so it serves a purpose still.  
...
extra.credit = time[z1]

```

```{r}
RandomWalk.extracredit = "
model{
...
j.model.2   <- jags.model (file = textConnection(RandomWalk.extracredit),
                             data = data2,
                             inits = init,
                             n.chains = 3)
...
out.2 <- as.matrix(jags.out.2)
```

```
ci.2 <- apply(exp(out.2[,3:ncol(out.2)]),2,quantile,c(0.025,0.5,0.975))
```

```{r}
plot(extra.credit,ci[2,],type='n',ylim=range(y2,na.rm=TRUE),ylab="Flu Index",log='y',xlim=range(extra.credit, na.rm=TRUE), xlab="Time") 

if(diff(time.rng) < 100){ 
  axis.Date(1, at=seq(extra.credit[time.rng[1]],extra.credit[time.rng[2]],by='month'), format = "%Y-%m")
}
ciEnvelope(extra.credit,ci[1,],ci[3,],col="purple")
points(extra.credit,y2,pch="~",cex=0.5)
```

* Generate a time-series plot for the CI of x that includes the observations (as above but zoom the plot on the last ~80 observations). 
  Use a different color and symbol to differentiate observations that were included in the model versus       those that were converted to NA's.
```{r}
z2 = 1:620
w2 = seq(540, 620, by=1)
...
extra.credit.2 = time[z2]

```

```{r}
RandomWalk.extracredit.3 = "
model{
...
"
data3 <- list(y1=log(y1),n=length(y1),x_ic=log(1000),tau_ic=100,a_obs=1,r_obs=1,a_add=1,r_add=1)

j.model.3   <- jags.model (file = textConnection(RandomWalk.extracredit.3),
                             data = data1,
                             inits = init,
                             n.chains = 3)

jags.out.3   <- coda.samples (model = j.model.3,
                            variable.names = c("tau_add","tau_obs"),
                                n.iter = 10000)

out.3 <- as.matrix(jags.out.3)
```

```
ci.3 <- apply(exp(out.3[,3:ncol(out.3)]),2,quantile,c(0.025,0.5,0.975))
```

```{r}
plot(extra.credit.2,ci[2,],type='n',ylim=range(y3,na.rm=TRUE),ylab="Flu Index",log='y',xlim=range(extra.credit.2, na.rm=TRUE), xlab="Time") 
if(diff(time.rng) < 100){ 
  axis.Date(1, at=seq(extra.credit.2[time.rng[1]],extra.credit.2[time.rng[2]],by='month'), format = "%Y-%m")
}
ciEnvelope(extra.credit.2,ci[1,],ci[3,],col="red")
points(extra.credit.2,y3,pch="!",cex=0.5)
```
...
Ending Notes
______________________________________________________________________________________________I worked with Erin Freeman on this lab.

The reason why my confidence intervals do not work is because of their length.  The matrix out.1, out.2, and out.3 area all 60000 long when they need to be 18660000 long, as in the example run.  Even though Erin and I used the same methods of copying and editing the code, somehow mine does not have the desired length.  Once I figure out why the length is so much shorter, the individual CI's could be created and compared to one another.   
