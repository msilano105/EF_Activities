...
ci.25 = apply(exp(out.1[,3:ncol(out.1)]),2,quantile,c(0.5))
```

```{r}
plot(time,ci.1[2,],type='n',ylim=range(y1,na.rm=TRUE),ylab="Flu Index",log='y',xlim=range(time, na.rm=TRUE), xlab="Time") 
if(diff(time.rng) < 100){ 
  axis.Date(1, at=seq(time[time.rng[1]],time[time.rng[2]],by='month'), format = "%Y-%m")
}
ciEnvelope(time,ci.1[1,],ci.1[3,],col="green")
points(time,y1,pch="*",cex=0.5)
```
...
```{r}
plot(y, ci.25, ylab="Median Predicted", xlab="Observed", log='y')
```

* Comment on the accuracy and precision of the estimates.
```
The estimates in the model with 3/4 missing data are still somewhat accurate to the observed values.  Even though the so much data is missing the model is still capable of creating an accurate confidence interval.  The CI was much larger than with the data so its precision was lost to essentially educated guessing.
```
...
```{r}
plot(time,ci.2[2,],type='n',ylim=range(y2,na.rm=TRUE),ylab="Flu Index",log='y',xlim=range(time, na.rm=TRUE), xlab="Time") 

if(diff(time.rng) < 100){ 
  axis.Date(1, at=seq(time[time.rng[1]],time[time.rng[2]],by='month'), format = "%Y-%m")
}
ciEnvelope(time,ci.2[1,],ci.2[3,],col="purple")
...
```{r}
plot(time,ci.3[2,],type='n',ylim=range(y3,na.rm=TRUE),ylab="Flu Index",log='y',xlim=range(time, na.rm=TRUE), xlab="Time") 

if(diff(time.rng) < 100){ 
  axis.Date(1, at=seq(time[time.rng[1]],time[time.rng[2]],by='month'), format = "%Y-%m")
}
ciEnvelope(time,ci.3[1,],ci.3[3,],col="red")
...
```
The models were spectaucal until they came to the point with mising data. once they encountered 40 or 80 missing data points, they had no idea what was going on and created a CI so large it is useless.  

```


Ending Notes
________________________________________________________________________________I worked with Erin Freeman on this lab.
You assisted me in class as well as in your office hours.  
________________________________________________________________________________